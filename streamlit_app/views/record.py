...
from lib.other_assets_ui import render_wealth_forms  # noqa: E402
...
def main() -> None:
    ...
    if view == "OCR":
        ...
        st.caption("매매 · 배당 · 현금 · 순자산 · 부채 · 세금 수기 입력")
        tabs = st.tabs(["매매·배당·현금", "순자산", "부채", "세금"])
        with tabs[0]:
            render_flow_forms(client, user)
        with tabs[1]:
            render_wealth_forms(client, user)
        with tabs[2]:
